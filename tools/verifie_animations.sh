...
#
# ⚠️ REGARDE LE BOÎTIER aux étapes 2, 5 et 6. Les étapes 3 et 4 ne font que
# lire et régler : le boîtier y reste noir, et le script le dit. Le reste défile tout seul.
#
...
for c in ff0000:ROUGE 00ff00:VERT 0040ff:BLEU; do
    printf '   → %s\n' "${c#*:}"
    dis "light all ${c%%:*}" | grep -v '^    end$'
    sleep 2
done
pause "Le boîtier s'est-il allumé en rouge, vert, bleu ? (oui/non)" "preuve-de-vie"

echo
echo "   ⚠️ Les deux étapes suivantes ne font que LIRE et RÉGLER : le boîtier"
echo "   reste noir, c'est normal. Il se rallume à l'étape 5."

echo
echo "═══ 3. La géométrie mesurée, telle que le démon la porte ═══"
dis "geometry"
...
echo
echo "═══ 4. Elle se corrige, et elle survit au redémarrage ═══"
...
dis "animate vague couleur=00ff40" | grep -v '^    end$'
sleep 5
dis "animate off" | grep -v '^    end$'
pause "Était-ce bien vert ? (oui/non)" "couleur"

echo
echo "═══ 7. Ce qui est refusé l'est en le disant ═══"
for mauvais in \
    "animate bidule" \
...
echo
echo "═══ 8. Retour au repos ═══"
dis "light all 000000" | grep -v '^    end$'
echo "   Éteint."
