...
- **ADR-001** : Rust + Slint, zéro dépendance de runtime. Motif : OS immuable, exigence d'empreinte minimale au repos, et refus d'un empaquetage Flatpak dont le sandbox gênerait l'accès à `/dev/hidraw*`, `/dev/i2c-8` et l'USB brut.
  - **Révisé le 2026-08-01 (issue #23)**, à la première compilation de Slint sur SHYNAEL. Slint **lie fontconfig à la compilation** (`yeslogic-fontconfig-sys` → `pkg-config fontconfig`), ce qui réclame un paquet de développement qu'un OS immuable n'a pas — et que l'ADR-001 refuse d'ajouter par surcouche `rpm-ostree`. Décision : **feature `fontconfig-dlopen`**, qui charge la bibliothèque au démarrage au lieu de l'exiger à la construction. La promesse tient même mieux qu'espéré : si `libfontconfig.so.1` manque, la fenêtre ne casse pas, elle retombe sur la police embarquée dans le binaire.
    - **Contrepartie assumée, et elle est laide** : cette feature n'est pas exposée par le crate `slint`, il faut donc déclarer `i-slint-common` — un crate **interne** de Slint — en dépendance directe **uniquement pour l'activer**. Notre code ne l'importe jamais. Ce motif casse si Slint réorganise ses crates internes. **À réviser si** une montée de version de Slint échoue à résoudre cette dépendance, ou si `slint` finit par exposer la feature lui-même.
- **ADR-002** : démon et interface séparés, IPC par socket Unix. Motif : l'éclairage doit vivre sans fenêtre ouverte, et un seul processus doit détenir les bus.
  ...
  - Le socket est en `0770 root:reverb` (le bit `x` est sans objet sur un socket), obtenu par `User=root` + `Group=reverb` + `UMask=0007` dans l'unité systemd — **sans `chown` après coup**, donc sans fenêtre pendant laquelle il serait ouvert à tous. C'est une entorse assumée au principe retenu pour udev (`uaccess` plutôt qu'un groupe) : ce socket n'expose que l'éclairage et les ventilateurs, là où un `GROUP=` sur `/dev/hidraw*` exposait la lecture brute de tous les périphériques HID. **À réviser si** Reverb tourne un jour sur une machine multi-utilisateurs.
  - **L'écran du Kraken restait hors du démon**, pour que `reverb screen --image` continue de marcher : une image de 1,2 Mo n'est pas exposable sur un protocole texte, et la prendre sans l'exposer serait une régression de capacité pour rien.
    - **Révisé le 2026-08-02 (issue #33)**, la condition posée s'étant réalisée : « il rejoindra le démon quand la fenêtre en aura besoin ». Décision : **le démon prend l'écran**. La fenêtre n'ouvre aucun périphérique, elle envoie donc un **chemin de fichier** et le démon lit lui-même — le mégaoctet ne traverse jamais le socket, seul le chemin le fait.
    - La régression de capacité annoncée a bien lieu — `reverb screen --image` en direct ne marche plus démon démarré, le nœud USB ne se réclamant pas deux fois — et elle est **compensée** : la ligne de commande passe par le socket comme la fenêtre, et y gagne le PNG, le JPEG et le GIF qu'elle n'avait pas. Ce qu'elle perd, c'est le fichier BGR brut, qui n'était qu'un pis-aller faute de décodeur.
    - Seule `screen --mire` reste en direct : outil de diagnostic, hors protocole, et donc refusée quand le démon tourne — comme `set`, `paint` et `ram`.
...
- **ADR-004** : transferts bulk vers l'écran par les `ioctl` d'usbfs plutôt que `rusb`/`libusb`, et `unsafe_code` abaissé de `forbid` à `deny` pour le permettre. Motif : même raisonnement que l'ADR-003 étendu à l'USB brut — `rusb` traînerait libusb en dépendance C, or l'ADR-001 pose le zéro dépendance de runtime. **Contrepartie assumée** : la bibliothèque standard n'expose aucun `ioctl`, donc la dérogation est déclarée en tête des modules concernés. Partout ailleurs un `unsafe` reste une erreur de compilation.
  ...
  - Surface totale : **cinq appels, tous à `ioctl`, tous sur un descripteur ouvert par la bibliothèque standard** — quatre dans `usbfs.rs`, un dans `i2c.rs`.
  ...
  - **Étendu le 2026-08-15 (issue #98)** : `USBDEVFS_RESET`, le cinquième appel, et le premier qui ne serve pas à afficher quelque chose. La condition de révision **ne déclenche pas** — c'est encore un `ioctl` sur un descripteur ouvert par `std`, et `rusb` n'apporterait rien qu'une bibliothèque C. Ce qui change n'est pas la dette, c'est le **geste** : il fait disparaître puis réapparaître un périphérique sur le bus, là où les quatre autres écrivaient. D'où une décision bornée par-dessus (`reverb-daemon::reparation`) plutôt qu'un appel libre.
- **ADR-005** : décodage PNG/JPEG/GIF par le crate `image`, sans ses fonctionnalités par défaut. **Introduit le 2026-08-02 (issue #33)**, et c'est la première dépendance du projet qui ne soit ni Slint ni une couture d'E/S.
  - Motif : l'issue demande qu'« une image PNG ou JPEG de n'importe quelle taille » et qu'un GIF soient acceptés. Écrire trois décodeurs à la main — dont l'inflate de PNG et la DCT de JPEG — serait un projet en soi, et un projet dont chaque défaut serait une lecture hors bornes sur un fichier venu du disque.
  - **Ce que l'ADR-001 refuse, c'est une bibliothèque *système*, pas une dépendance.** `image` est du Rust pur : `default-features = false` avec `png`, `jpeg` et `gif` seulement, aucun `build.rs` qui compile du C, rien à lier, rien à installer sur la machine. Le binaire reste unique et ne casse pas à une montée d'image de Bazzite — c'est la promesse à tenir, et elle tient.
  - Contrepartie **mesurée** : `reverbd` en `--release` passe de 1 134 912 à 2 021 328 octets, soit **+0,85 Mo** — décodeurs et dessin du cadran compris. Et une surface d'attaque qui n'existait pas — un décodeur d'image lit un fichier hostile. Elle est bornée par le fait que le fichier est nommé par l'utilisateur lui-même, jamais reçu du réseau.
  - **À réviser si** `image` se met à exiger une bibliothèque système pour l'un de ces trois formats, ou si le projet n'a plus besoin que d'un seul d'entre eux — un décodeur PNG seul se réécrirait, les deux autres non.
