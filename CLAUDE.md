...
- **ADR-004** : transferts bulk vers l'écran par les `ioctl` d'usbfs plutôt que `rusb`/`libusb`, et `unsafe_code` abaissé de `forbid` à `deny` pour le permettre. Motif : même raisonnement que l'ADR-003 étendu à l'USB brut — `rusb` traînerait libusb en dépendance C, or l'ADR-001 pose le zéro dépendance de runtime. **Contrepartie assumée** : la bibliothèque standard n'expose aucun `ioctl`, donc le seul `unsafe` du dépôt vit dans `crates/reverb-cli/src/usbfs.rs`, trois appels, dérogation déclarée en tête de module. Partout ailleurs un `unsafe` reste une erreur de compilation. **À réviser si** un second module réclame la dérogation : ce serait le signe que `rusb` coûte moins cher que la dette d'`unsafe` accumulée.
